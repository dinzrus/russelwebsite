...
export const handler: Handlers<FormData> = {
  ...
  async POST(req, ctx) {
    ...
    if (!title) {
      return ctx.render({
        slug, title, description, tags: tagsStr, liveUrl, githubUrl, featured, seoTitle, metaDescription, body,
...
export default function NewProject({ data }: { data: FormData }) {
  return (
    <DashboardLayout title="New Project" activeNav="projects">
